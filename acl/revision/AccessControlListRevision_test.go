...
func TestAccessControlListRevisionUnmarshal(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "ACL_REVISION",
			input:    []byte{revision.ACL_REVISION, 0x00, 0x00, 0x00},
			expected: revision.ACL_REVISION,
			wantSize: 1,
		},
		{
			name:     "ACL_REVISION_DS",
			input:    []byte{revision.ACL_REVISION_DS, 0x00, 0x00, 0x00},
			expected: revision.ACL_REVISION_DS,
			wantSize: 1,
		},
		{
...
func TestAccessControlListRevisionMarshal(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "ACL_REVISION",
			revision: revision.ACL_REVISION,
			expected: []byte{revision.ACL_REVISION},
		},
		{
			name:     "ACL_REVISION_DS",
			revision: revision.ACL_REVISION_DS,
			expected: []byte{revision.ACL_REVISION_DS},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			aclrev := revision.AccessControlListRevision{Value: tt.revision}
