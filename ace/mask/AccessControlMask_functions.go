...
//
// Parameters:
// - other: The other AccessControlMask to compare with
//
// Returns:
// - bool: true if the AccessControlMasks are equal, false otherwise
func (acm *AccessControlMask) Equal(other *AccessControlMask) bool {
	if acm == nil || other == nil {
		return acm == other
	}

	// Compare RawValue
	if acm.RawValue != other.RawValue {
		return false
	}

	// Compare RawBytes
	if !bytes.Equal(acm.RawBytes, other.RawBytes) {
		return false
	}

	// Compare RawBytesSize
	if acm.RawBytesSize != other.RawBytesSize {
		return false
	}

	// Compare Values
	if !slices.Equal(acm.Values, other.Values) {
		return false
	}

	// Compare Flags
	if !slices.Equal(acm.Flags, other.Flags) {
		return false
	}
